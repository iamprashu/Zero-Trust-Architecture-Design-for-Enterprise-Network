...
const bootstrapSuperadmin = async () => {
  try {
    ...
    if (!adminEmail || !adminPassword) {
      console.log(
        "Skipping superadmin setup: SUPERADMIN_EMAIL or SUPERADMIN_PASSWORD not set.",
      );
      return;
    }
    ...
    if (!existingAdmin) {
      console.log(
        "No superadmin found. Creating from environment variables...",
      );
...
const bootstrapSeedData = async () => {
  try {
    const permissions = [
      ...
      {
        name: "CREATE_LOAN_TRANSACTION",
        description: "Can create loan transactions",
      },
    ];
    ...
    const tellerRole = await Role.findOneAndUpdate(
      ...
      {
        $set: {
          permissions: ["READ_ACCOUNT", "CREATE_TRANSACTION", "TRANSFER_MONEY"],
        },
      },
      { upsert: true, new: true },
    );
    ...
    const userRole = await Role.findOneAndUpdate(
      ...
      {
        $set: {
          permissions: ["READ_TRANSACTION", "READ_ACCOUNT", "TRANSFER_MONEY"],
        },
      },
      { upsert: true, new: true },
    );
